...
1. 如果 mouse=0，老鼠躲入洞里，则老鼠获胜，因此对于任意cat 和 turns 都有`dp[0][cat][turns]=1`，该状态为老鼠的必胜状态，猫的必败状态。


1. 如果 cat=mouse，猫和老鼠占据相同的节点，则猫获胜，因此当cat=mouse 时，对于任意 mouse、cat 和turns 都有`dp[mouse][cat][turns]=2`，该状态为老鼠的必败状态，猫的必胜状态。注意猫不能移动到节点 00，因此当 mouse=0 时，一定有 cat≠mouse 

2. 如果 turns≥2n，则是平局，该状态为双方的必和状态。
